...
class RRT:
    ...
    def smooth_paths(self): 
        ...
        d , final_angle = self.calc_distance_and_angle(self.start,self.goal)
        angles = [self.start.yaw]
        self.start.yaw = -np.pi/2
        ...
        for i in range(len(self.smoothed_path)-1):
            ...
            path_db = dubins.shortest_path((self.smoothed_path[i].x ,self.smoothed_path[i].y , self.smoothed_path[i].yaw), (self.smoothed_path[i+1].x ,self.smoothed_path[i+1].y , self.smoothed_path[i+1].yaw),1)
