...
class RRT:
    def __init__(self,svc,k,q,p,dominion = [-10,10,-10,10] ,max_time = 0.1, is_RRT_star = True):
        ...
        self.max_time = 6 # max time for the search
    ...
    def cost_optimal_parent(self,qnew,current_parent):
        ...
        if(not nodes_with_in_radius): # return the nearest node
            ...
            for node in nodes_with_in_radius:
                new_node_cost = self.nodes_list[node] + qnew.get_distance(node)
    ...
    def rewire(self,qnew):
        ...
        for node in  nodes_with_in_radius:
            ...
            if(new_node_cost < self.nodes_list[node] and self.svc.check_path([[node.x,node.y],[qnew.x , qnew.y]])):
                node.parent = qnew
                self.edges.remove((node.parent,node))
                self.edges.append((qnew,node))
                self.nodes_list[node] = new_node_cost
    ...
    def compute_path(self , start , goal):
        ...
        for k in range(self.k):
            ...
            if((time.time() - self.start_time) > self.max_time and not  self.goal_found ):
                self.max_time += 0.1 # give additional time to search
            else:
                break # exit the loop if the max time is reached and goal reached 
        if(self.goal_found):
