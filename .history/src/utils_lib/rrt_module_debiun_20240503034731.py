...
class RRT:
    ...
    def reconstract_db_path(self):
        ...
        self.path_db = [self.goal]
        self.dubin_path = []
        self.dubin_path2 = []
        node = self.goal
    
        while node.parent:
            for (ix, iy) in (reversed(node.path)):
...
# 
            for (ix, iy ,yaw) in (reversed(node.waypoints[0])):
                self.dubin_path2.append([ix, iy])
                # print("path" , path)
     
            node = node.parent
            self.path.append(node)
        #
        print("final_path cost" , self.dubin_path)
