...
class Planner2:
    def  __init__(self, state_validity_checker, start, goal, prob_to_goal=0.4, del_q=5, max_iterations=10000, dominion=[-5, 5, -5, 5]):
        # define constructor ...
        self.max_iteration=max_iterations
        self.goal=[goal[0],goal[1]]
        self.state_validity=state_validity_checker
    ...
    def compute_path(self):
        ...
        for iter in range(self.max_iteration):
            ...
            if self.state_validity.check_path([q_near.pos,q_new.pos]):
                ...
                if self.goal_reached and self.max_it_time < time() - it_time:
                    break
        ...
        return self.smooth_path, self.distance
...
# StateValidityChecker class has to be used. The planning dominion must be specified as well as the maximum planning time.
# The planner returns a path that is a list of poses ([x, y]).
def compute_path2(start_p, goal_p, state_validity_checker, dominion, max_iterations=1000):
