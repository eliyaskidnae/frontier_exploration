...
class RRT:
    ...
    def smooth_path(self):
        ...
        while True:
            ...
            if self.smoothed_path[len(self.smoothed_path)-1] == self.start:
                break
            counter +=1        # incase the max iteration is reached we can't smooth the path return the path 
    ...
    def compute_path(self , start , goal):
        ...
        self.vertices.add(self.start) # initialize the vertices list 
        ...
        for k in range(self.k):
            ...
            # print("time",time.time() - self.start_time , self.goal_found)
            if((time.time() - self.start_time) > self.max_time and self.goal_found == True):
