...
def compute_path(start_p, goal_p, svc, bounds , max_time=7.0):
    ...
    rrt = RRT(svc ,100 ,1, 0.2 , bounds, max_time )
    # rrt = RRT_Debiun(svc ,2000 ,3, 0.2 , bounds, max_time )
...
def move_dwa(current, goal ,  svc):
    # input [forward speed, yaw_rate]
    # [x,y ,t,v,w]
    global local_map
    
    
    u, pre_traj , all_traj = dwa_control(current, config, goal,  svc)
    # print("Trajectory", predicted_trajectory)
    return u  , pre_traj , all_traj
